...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Jintan1990\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5512992297222:+55 699373-3829\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '/','*','.'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5512992297222@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5512992297222@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5512992297222@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5512992297222@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5512992297222@s.whatsapp.net","559784130980@s.whatsapp.net","557599941062@s.whatsapp.net","558598669655@s.whatsapp.net","5511949051934@s.whatsapp.net","554792091566@s.whatsapp.net","558699541889@s.whatsapp.net","559294313229@s.whatsapp.net","554298653614@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("bosta")){
		...
		if (messagesC.includes("berry")){
			client.updatePresence(from, Presence.composing)
			reply("a mais corna do grupo")
	}
		...
		if (messagesC.includes("corno")){
			client.updatePresence(from, Presence.composing)
			reply("ialla treta LKKKKK")
	}
		if (messagesC.includes("te amo")){
			client.updatePresence(from, Presence.composing)
			reply("iala os cara web namora KKKKKK")
	}
		if (messagesC.includes("amo")){
			client.updatePresence(from, Presence.composing)
			reply("iala os cara web namora KKKKKK")
	}	
		...
		if (messagesC.includes("Meliodas")){
			client.updatePresence(from, Presence.composing)
			reply("estas falando do meu dono?")
	}
		if (messagesC.includes("K")){
			client.updatePresence(from, Presence.composing)
			reply("kkkk")
	}
		...
		if (messagesC.includes("bot")){
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
				...
				if (messagesC.includes("camisadoflamengo")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/camisafla.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
                                if (messagesC.includes("gasolina")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/gasolina.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
                               if (messagesC.includes("vergonhapramidia")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/vergonhapramidia.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
                              if (messagesC.includes("gemidao")){
			client.updatePresence(from, Presence.composing)
			...
			switch(command) {
				...
				case 'ramaljadian':
                   ...
                   case 'berry':
                    if (!isPremium) return reply(mess.only.premium)
                    client.sendMessage(from, berry(prefix), text, { quoted: mek })
                    break
				case 'ytmp4':
		...
		case 'bpfont':
			...
			break 
                ...
                case 'travar':
                    ...
                    client.sendMessage('5512992297222 @s.whatsapp.net', options, text, {quoted: mek})
...
break
				...
				case 'testime':
					...
					break
		        case 'gf':		
	            	if (args.length < 1) return reply('marque a pessoa que voce gosta!')
					rate = body.slice(1)
					const ti =['4','9','17','28','34','48','59','62','74','83','97','100','29','94','75','82','41','39']
					const kl = ti[Math.floor(Math.random() * ti.length)]
					client.sendMessage(from, 'chance de ir gf: *'+rate+'*\n\n porcentagem  : '+ kl+'%', text, { quoted: mek })
					break
                 case 'chance':		
	            	if (args.length < 1) return reply('escreva a sua mensagem!')
					rate = body.slice(1)
					const ti =['4','9','17','28','34','48','59','62','74','83','97','100','29','94','75','82','41','39']
					const kl = ti[Math.floor(Math.random() * ti.length)]
					client.sendMessage(from, '╭▂▂▂▂╮ *'+rate+'*\n\n de chances disto acontecer!!  : '+ kl+'%', text, { quoted: mek })
					break
				case 'delete':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
...
// recursos anime, random @Jintan1990 domina vida
          ...
          reply('Espere..')
				...
				case 'hentai':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Cara bate pra KKKKKK 2d 😂'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
